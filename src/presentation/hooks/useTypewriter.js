 ...
 * @param {number} typingSpeed - Speed of typing in milliseconds (default: 100)
 * @param {number} blinkSpeed - Speed of blinking in milliseconds (default: 500)
 * @param {number} blinkCount - Number of times to blink (default: 5)
        ...
        setShowLastChar((prev) => {
          ...
          if (newState) {
            setBlinkIteration((prev) => {
              const next = prev + 1
              // After 5 blinks, reset to typing phase
              if (next >= blinkCount) {
                setPhase('typing')
                setCurrentIndex(0)
                setDisplayedText('')
